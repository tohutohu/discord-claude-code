...
Deno.test("WorkspaceManager - ThreadInfoにdevcontainerConfigが含まれる", async () => {
  const { workspace, cleanup } = await createTestWorkspaceManager();
  ...
  try {
    ...
    assertEquals(loaded?.devcontainerConfig?.isStarted, true);
  } finally {
    await cleanup();
  }
});

Deno.test("WorkspaceManager - devcontainerConfigがnullでも正常に保存・読み込みできる", async () => {
  const { workspace, cleanup } = await createTestWorkspaceManager();
  ...
  try {
    ...
    assertEquals(loaded?.devcontainerConfig, null);
  } finally {
    await cleanup();
  }
});
