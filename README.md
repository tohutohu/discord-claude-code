...
Botのコードを最新版に更新する機能を提供しています。

#### 使い方

Discordで `/update` コマンドを実行すると：

1. ローカルの変更を自動的にstashに保存
2. リモートリポジトリから最新のコードを取得
3. マージを試行（コンフリクトがある場合は手動解決が必要）
4. 保存した変更を復元

#### 注意事項

- 更新後はBotの再起動が必要です
- HMR（Hot Module Replacement）を有効にしている場合は自動的に反映されます
- コンフリクトが発生した場合は手動で解決する必要があります
