...
Claude Code実行の中断機能を提供するスラッシュコマンド。

**用途:**

- 実行中のClaude Codeプロセスを中断
- 長時間実行や無限ループ状態の強制終了
- 誤った指示を与えた場合の即座の停止
...
- **スレッド管理権限（Manage Threads）** が必要
- この権限を持つユーザーのみがコマンドを使用可能
- 通常、スレッドの作成者やサーバー管理者が該当

**動作:**

- 実行中のClaude CLIプロセスをgraceful shutdownで停止
- 子プロセスも含めて確実に終了
- 中断された旨のメッセージをスレッドに送信

**中断後の継続:**

- 新しい指示を送信することで作業を継続可能
- 中断前の状態（ファイル変更など）は保持される
- セッションログに中断イベントが記録される
